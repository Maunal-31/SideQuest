...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Welcome back, ${user.displayName || 'Hunter'}! 🚀`);
      navigate('/');
    } catch (error: any) {
      ...
      <div className="bg-white w-full max-w-md rounded-2xl p-8 brutal-border shadow-[8px_8px_0_0_rgba(0,0,0,1)] relative">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            ...
            className="w-full py-4 bg-[#EAB308] hover:bg-black hover:text-white text-black font-black uppercase tracking-wider rounded-xl transition-all brutal-border brutal-shadow brutal-shadow-hover text-lg flex items-center justify-center gap-2 mt-6 disabled:opacity-50"
          >
            {isSubmitting ? (
              <>
              ...
              <>
                Sign In <LogIn className="w-5 h-5" strokeWidth={3} />
              </>
            )}
          </button>
        </form>
        ...
        <div className="mt-8 pt-6 border-t-4 border-black text-center">
          <p className="text-sm font-bold text-black">
            ...
            <Link to="/signup" className="text-[#16A34A] hover:underline font-black uppercase">
              Create Account
            </Link>
          </p>
        </div>
